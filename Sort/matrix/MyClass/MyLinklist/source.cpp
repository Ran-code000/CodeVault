...
protected:
    ListNode* head;
public:
    LinkList(ListNode* p = nullptr) : head(p){}
    ~LinkList();
...
LinkList* mergeLinkList(LinkList& a, LinkList& b){
    ...
    if(a.getType() && !b.getType()) b.reverseAll();
    else if(!a.getType() && !b.getType()) {a.reverseAll(); b.reverseAll();}
    else if(!a.getType() && b.getType()) a.reverseAll();
    //a.display();
    //b.display();
    ListNode* pa = a.head;
    ListNode* pb = b.head;

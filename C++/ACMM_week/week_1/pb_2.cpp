...
namespace jyy{
	...
	IN int ksm(int x,int y){int tmp=1;for(;y;y>>=1,x=1ll*x*x%mod)if(y&1)tmp=1ll*tmp*x%mod;return tmp;}
	...
	int ifac[N],fac[N];
	IN void init(int x){fac[0]=ifac[0]=1;for(int i=1;i<=x;i++)fac[i]=1ll*fac[i-1]*i%mod;ifac[x]=inv(fac[x]);for(int i=x-1;i>=1;i--)ifac[i]=1ll*ifac[i+1]*(i+1)%mod;}
	IN int C(int x,int y){return (x<y||y<0||x<0)?0:1ll*fac[x]*ifac[y]%mod*ifac[x-y]%mod;}
}
using namespace jyy;
int a[N],b[N],siz[N],ans[N];
int main(){
	int n=read();
	ll res=0;
	for(int i=1;i<=n;i++)a[i]=read();
	for(int i=1;i<=n;i++)b[i]=read();
	for(int i=1;i<=n;i++)siz[read()]++;
	for(int i=1;i<=n;i++)ans[b[i]]+=siz[i];
	for(int i=1;i<=n;i++)res+=ans[a[i]];
	cout<<res<<endl;
}

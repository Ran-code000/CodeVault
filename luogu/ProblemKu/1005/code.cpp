...
HP max(const HP &a, const HP &b) {
	...
	for (int i = a.len; i > 0; i--) if (a.p[i] > b.p[i]) return a; else if (a.p[i] < b.p[i]) return b;
	return a;
} //比较取最大值 
void BaseTwo() {
	base[0].p[1] = 1, base[0].len = 1;
	for (int i = 1; i <= m + 2; i++) base[i] = base[i - 1] * 2;
} //预处理出2的幂 
int main() {
	scanf("%d%d", &n, &m);
	BaseTwo();
	while (n--) {
		...
		HP Max;
		for (int i = 1; i <= m; i++) Max = max(Max, f[i][i] + base[m] * ar[i]);
		ans = ans + Max; //记录到总答案中 
	}
	ans.print(); //输出 
	return 0;
